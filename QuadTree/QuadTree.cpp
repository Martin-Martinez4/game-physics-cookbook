...
int QuadTreeNode::NumObjects(){
  ...
  while(process.size() > 0){
    ...
    if(!processing->IsLeaf()){
      for(int i = 0, size = processing->childern.size(); i < size; ++i){
        process.push(&processing->childern[i]);
      }
    }else{
...
void QuadTreeNode::Insert(QuadTreeData& data){
  if(!RectangleRectangle(data.bounds, nodeBounds)){
    // The object does not fit into the node
    return; 
  }
  ...
  if(IsLeaf()){
    ...
    for(int i = 0, size = childern.size(); i < size; ++i){
      childern[i].Insert(data);
    }
  }
}
...
void QuadTreeNode::Split(){
  ...
  Rectangle2D childAreas[] = {
    ...
    Rectangle2D(FromMinMax(vec2(center.x, center.y), vec2(max.x, center.y))),
    Rectangle2D(FromMinMax(vec2(min.x, center.y), vec2(center.x, max.y))),
  };

  for(int i = 0; i < 4; ++i){
    childern.push_back(QuadTreeNode(childAreas[i]));
    childern[i].currentDepth = currentDepth + 1;
  }
  for(int i = 0, size = contents.size(); i < size; ++i){
    childern[i].Insert(*contents[i]);
  }
  contents.clear();
}
...
std::vector<QuadTreeData*> QuadTreeNode::Query(const Rectangle2D& area){
  ...
  if(IsLeaf()){
    ...
    for(int i = 0, size = childern.size(); i < size; ++i){
      std::vector<QuadTreeData*> recurse = childern[i].Query(area);
